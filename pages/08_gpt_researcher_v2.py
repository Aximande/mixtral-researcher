...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'gpt_researcher_local')))
...
def main():
    st.set_page_config(
        page_title="GPTResearcher Report Generator", page_icon=":memo:", layout="wide"
    )
    st.markdown(
    ...
    research_option = st.radio(
        "Select research option:", ("Web Search", "Specific URLs")
    )
    sources = None
    if research_option == "Specific URLs":
        urls = st.text_area("Enter URLs (one per line):")
        sources = validate_urls(
